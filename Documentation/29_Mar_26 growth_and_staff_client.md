...
```mermaid
graph TD
    A[Admin/Owner] -->|Full Access| B(Reports & Settings)
    C[Manager] -->|Limited Access| D(Orders & Staff)
    E[Cashier] -->|Restricted Access| F(POS & Sales Only)
```
...
```mermaid
graph LR
    A[Facebook Ad] -- Click --> B(Your Website)
    B -- Purchase --> C{Sale Tagged: Summer Sale}
    C -- Report --> D[Profit: +45,000 BDT]
```
