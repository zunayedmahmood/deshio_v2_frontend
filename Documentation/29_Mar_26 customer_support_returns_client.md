...
```mermaid
graph TD
    A[Customer Request] --> B{Manager Approval}
    B -->|Rejected| C[Return Denied]
    B -->|Approved| D[Quality Check]
    D -->|Damaged by Customer| C
    D -->|Perfect Condition| E[Restock & Refund/Exchange]
    D -->|Manufacturer Defect| F[Move to Defective Stock]
```
...
```mermaid
graph LR
    A[Active Data] -- Delete --> B((Recycle Bin))
    B -- 7 Days --> C[Permanently Gone]
    B -- Restore --> A
```
