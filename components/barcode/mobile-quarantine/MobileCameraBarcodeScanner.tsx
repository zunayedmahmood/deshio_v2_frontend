...
// TEMPORARY / QUARANTINED FEATURE:
// This component contains camera-only concerns. It intentionally knows nothing about POS,
// orders, dispatches, returns, or inventory business rules; callers pass their existing scan
// handler through onScan. That makes this feature safe to remove later without touching those rules.
      ...
      try {
        ...
        scanner.render(
          (decodedText: string) => {
            ...
            if (!barcode || scanInFlightRef.current) return;
            ...
            Promise.resolve(onScanRef.current(barcode))
              .catch((error) => {
                // Business-rule errors are normally surfaced by the caller's existing scan handler.
                console.warn('Mobile camera scan handler rejected barcode:', error);
              })
              .finally(() => {
                window.setTimeout(() => {
