...
const SectionHeader: React.FC<SectionHeaderProps> = ({ eyebrow, title, subtitle, actionLabel, onAction }) => (
  <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
    <div>
      {eyebrow && (
        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--cyan)] mb-3" style={{ fontFamily: "'DM Mono', monospace" }}>
          {eyebrow}
        </p>
      )}
      <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 500, letterSpacing: '-0.02em', lineHeight: 1, color: 'var(--text-primary)' }}>
        {title}
      </h2>
      {subtitle && (
        <p className="mt-3 text-[14px] max-w-lg text-[var(--text-secondary)]" style={{ fontFamily: "'Jost', sans-serif" }}>
          {subtitle}
        </p>
      )}
    </div>
