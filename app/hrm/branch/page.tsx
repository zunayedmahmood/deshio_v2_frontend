    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Manage daily arrivals and departures</p>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-gray-200 bg-white py-2 pl-9 pr-4 text-sm text-gray-900 shadow-sm outline-none placeholder:text-gray-400 focus:border-gray-400 focus:ring-2 focus:ring-gray-200 md:w-64 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:placeholder:text-gray-500 dark:focus:border-gray-500 dark:focus:ring-gray-700"
                  />
                </div>
                <button className="rounded-xl border border-gray-200 bg-white p-2 text-gray-500 shadow-sm transition-all hover:bg-gray-50 hover:text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-gray-100">
                  ...
                  {filteredEmployees.map((emp) => {
                    ...
                    return (
                      <tr key={emp.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/20 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          {record ? (
                            <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${record.status?.toLowerCase() === 'absent' ? 'border border-red-100 bg-red-50 text-red-600 dark:border-red-900/60 dark:bg-red-950/40 dark:text-red-300' :
                                record.status?.toLowerCase() === 'leave' ? 'border border-orange-100 bg-orange-50 text-orange-600 dark:border-orange-900/60 dark:bg-orange-950/40 dark:text-orange-300' :
                                  record.status?.toLowerCase() === 'half_day' ? 'border border-yellow-100 bg-yellow-50 text-yellow-600 dark:border-yellow-900/60 dark:bg-yellow-950/40 dark:text-yellow-300' :
                                    record.status?.toLowerCase() === 'late' ? 'border border-amber-100 bg-amber-50 text-amber-600 dark:border-amber-900/60 dark:bg-amber-950/40 dark:text-amber-300' :
                                      record.status?.toLowerCase() === 'holiday_auto' || record.status?.toLowerCase() === 'off_day_auto' ? 'border border-purple-100 bg-purple-50 text-purple-600 dark:border-purple-900/60 dark:bg-purple-950/40 dark:text-purple-300' :
                                        'border border-green-100 bg-green-50 text-green-600 dark:border-green-900/60 dark:bg-green-950/40 dark:text-green-300'}`}>
                            ...
                            <span className="rounded-full bg-gray-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:bg-gray-700 dark:text-gray-300">
                              Not Marked
                            </span>
                          )}
                        </td>
                            ...
                            <div className="relative">
                              <AccessControl roles={['super-admin', 'admin', 'branch-manager']}>
                                <button
                                  ...
                                  className={`rounded-lg p-2 transition-colors ${activeMenuId === Number(emp.id) ? 'bg-gray-100 text-gray-900 dark:bg-gray-700 dark:text-gray-100' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-200'}`}
                                >
                                ...
                                <>
                                  ...
                                  <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 py-2 z-50 animate-in fade-in zoom-in duration-200">
                                    ...
                                    <button
                                      ...
                                      className="flex w-full items-center gap-3 px-4 py-2.5 text-left text-sm font-bold text-orange-600 hover:bg-gray-50 dark:text-orange-400 dark:hover:bg-gray-700"
                                    >
                                    ...
                                    <button
                                      ...
                                      className="flex w-full items-center gap-3 px-4 py-2.5 text-left text-sm font-bold text-blue-600 hover:bg-gray-50 dark:text-blue-400 dark:hover:bg-gray-700"
                                    >
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-5">
              {(Array.isArray(performanceReport?.items) ? performanceReport.items : []).slice(0, 5).map((rank: any, idx: number) => (
                <div key={rank.employee.id} className="flex items-center gap-4">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm ${idx === 0 ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-950/50 dark:text-yellow-300' :
                    idx === 1 ? 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-200' :
          ...
          {/* Quick Actions Card */}
          <div className="rounded-2xl bg-black p-6 text-white shadow-lg shadow-black/20 dark:border dark:border-gray-700 dark:bg-gray-800 dark:shadow-none">
