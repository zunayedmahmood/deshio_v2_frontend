...
export default function HomepageSettingsPage() {
  ...
  const [showcaseProductSearch, setShowcaseProductSearch] = useState<Record<string, string>>({});
  const [showcaseSearchResults, setShowcaseSearchResults] = useState<Record<string, any[]>>({});
  const [showcaseSearching, setShowcaseSearching] = useState<Record<string, boolean>>({});
  ...
  const loadData = async () => {
    try {
      ...
      const normalized: HomepageSettings = {
        ...settingsData,
        ticker: {
          enabled: settingsData.ticker?.enabled ?? false,
        ...
        showcase: (settingsData.showcase || []).map((item: any) => ({
          ...
          product_ids_by_category: item.product_ids_by_category || {},
          product_data_by_category: item.product_data_by_category || {},
        })),
        new_arrivals: {
        ...
        instagram_reels: {
          enabled: settingsData.instagram_reels?.enabled ?? false,
          links: settingsData.instagram_reels?.links || [],
        },
        section_order: settingsData.section_order || ['hero', 'featured_collections', 'new_arrivals', 'bannered_collections', 'instagram_reels', 'showcase']
      };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (settings.showcase) {
        if (settings.showcase.length === 0) {
          ...
          settings.showcase.forEach((showcase, index) => {
            ...
            Object.entries(showcase.product_ids_by_category || {}).forEach(([categoryId, ids]) => {
              (ids as number[]).slice(0, 8).forEach((productId, productIndex) => {
                formData.append(`showcase[${index}][product_ids_by_category][${categoryId}][${productIndex}]`, String(productId));
              });
            });
          });
        }
      ...
      // Instagram Reels
      if (settings.instagram_reels) {
        formData.append("instagram_reels[enabled]", settings.instagram_reels.enabled ? "1" : "0");
        (settings.instagram_reels.links || []).forEach((link, index) => {
          formData.append(`instagram_reels[links][${index}]`, link);
        });
      }
  ...
  const addReelLink = () => {
    ...
    setSettings({
      ...settings,
      instagram_reels: {
        enabled: settings.instagram_reels?.enabled ?? true,
        links: [...(settings.instagram_reels?.links || []), ""],
      },
    });
  };

  const updateReelLink = (index: number, value: string) => {
    if (!settings) return;
    const links = [...(settings.instagram_reels?.links || [])];
    links[index] = value;
    setSettings({
      ...settings,
      instagram_reels: {
  ...
  const removeReelLink = (index: number) => {
    if (!settings) return;
    const links = (settings.instagram_reels?.links || []).filter((_, i) => i !== index);
    setSettings({
      ...settings,
      instagram_reels: {
        enabled: settings.instagram_reels?.enabled ?? false,
        links,
      },
    });
  };
  ...
  const addShowcase = () => {
    ...
    setSettings({
      ...settings,
      showcase: [...(settings.showcase || []), { category_id: flatCategories[0]?.id || 0, subcategories: [], product_ids_by_category: {}, product_data_by_category: {} }]
    });
  };
  ...
  const addSubcategoryToShowcase = (showcaseIndex: number) => {
    ...
    newShowcase[showcaseIndex].product_ids_by_category = newShowcase[showcaseIndex].product_ids_by_category || {};
    newShowcase[showcaseIndex].product_data_by_category = newShowcase[showcaseIndex].product_data_by_category || {};
    setSettings({ ...settings, showcase: newShowcase });
  };
  ...
  const updateSubcategoryInShowcase = (showcaseIndex: number, subIndex: number, newId: number) => {
    if (!settings) return;
    const newShowcase = [...(settings.showcase || [])];
    const oldId = newShowcase[showcaseIndex].subcategories[subIndex];
    newShowcase[showcaseIndex].subcategories[subIndex] = newId;
    if (oldId !== newId) {
      delete newShowcase[showcaseIndex].product_ids_by_category?.[String(oldId)];
      delete newShowcase[showcaseIndex].product_data_by_category?.[String(oldId)];
    }
    setSettings({ ...settings, showcase: newShowcase });
  };
  ...
  const removeSubcategoryFromShowcase = (showcaseIndex: number, subIndex: number) => {
    if (!settings) return;
    const newShowcase = [...(settings.showcase || [])];
    const removedId = newShowcase[showcaseIndex].subcategories[subIndex];
    newShowcase[showcaseIndex].subcategories.splice(subIndex, 1);
    delete newShowcase[showcaseIndex].product_ids_by_category?.[String(removedId)];
    delete newShowcase[showcaseIndex].product_data_by_category?.[String(removedId)];
    setSettings({ ...settings, showcase: newShowcase });
  };
  ...
  const getShowcaseTargets = (showcaseItem: any) => {
    const ids = [showcaseItem.category_id, ...(showcaseItem.subcategories || [])]
      .map((id: any) => Number(id))
      .filter(Boolean);
    return Array.from(new Set(ids));
  };

  const findCategoryTitle = (id: number) => flatCategories.find((category) => Number(category.id) === Number(id))?.title || `Category ${id}`;

  const searchShowcaseProducts = async (key: string, q: string) => {
    setShowcaseProductSearch((prev) => ({ ...prev, [key]: q }));
    if (!q || q.length < 2) {
      setShowcaseSearchResults((prev) => ({ ...prev, [key]: [] }));
      return;
    }
    setShowcaseSearching((prev) => ({ ...prev, [key]: true }));
    try {
      const response = await catalogService.getProducts({ search: q, group_by_sku: false, per_page: 20 });
      const displayProducts = response.grouped_products?.length
        ? response.grouped_products.map((gp) => gp.main_variant)
        : response.products;
      setShowcaseSearchResults((prev) => ({ ...prev, [key]: displayProducts || [] }));
    } catch (error) {
      console.error("Showcase product search failed:", error);
    } finally {
      setShowcaseSearching((prev) => ({ ...prev, [key]: false }));
    }
  };

  const toggleShowcaseProductSelection = (showcaseIndex: number, categoryId: number, product: any) => {
    if (!settings) return;
    const newShowcase = [...(settings.showcase || [])];
    const item: any = { ...newShowcase[showcaseIndex] };
    const key = String(categoryId);
    const productIdsByCategory = { ...(item.product_ids_by_category || {}) };
    const productDataByCategory = { ...(item.product_data_by_category || {}) };
    const currentIds = [...(productIdsByCategory[key] || [])];
    const currentProducts = [...(productDataByCategory[key] || [])];
    const existingIndex = currentIds.findIndex((id) => Number(id) === Number(product.id));

    if (existingIndex >= 0) {
      currentIds.splice(existingIndex, 1);
      const productIndex = currentProducts.findIndex((p) => Number(p.id) === Number(product.id));
      if (productIndex >= 0) currentProducts.splice(productIndex, 1);
    } else {
      if (currentIds.length >= 8) {
        toast.error("You can select up to 8 products for each category/subcategory");
        return;
      }
      currentIds.push(Number(product.id));
      currentProducts.push(product);
    }

    productIdsByCategory[key] = currentIds;
    productDataByCategory[key] = currentProducts;
    item.product_ids_by_category = productIdsByCategory;
    item.product_data_by_category = productDataByCategory;
    newShowcase[showcaseIndex] = item;
    setSettings({ ...settings, showcase: newShowcase });
  };

  const removeShowcaseProduct = (showcaseIndex: number, categoryId: number, productId: number) => {
    if (!settings) return;
    const newShowcase = [...(settings.showcase || [])];
    const item: any = { ...newShowcase[showcaseIndex] };
    const key = String(categoryId);
    item.product_ids_by_category = { ...(item.product_ids_by_category || {}) };
    item.product_data_by_category = { ...(item.product_data_by_category || {}) };
    item.product_ids_by_category[key] = (item.product_ids_by_category[key] || []).filter((id: number) => Number(id) !== Number(productId));
    item.product_data_by_category[key] = (item.product_data_by_category[key] || []).filter((p: any) => Number(p.id) !== Number(productId));
    newShowcase[showcaseIndex] = item;
    setSettings({ ...settings, showcase: newShowcase });
  };
              ...
              <div className="space-y-8">
                ...
                <section className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Instagram Reels</h2>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Add reel/post links here. The homepage will only render this section when enabled and at least one link exists.</p>
                    </div>
                    <label className="flex items-center gap-2 text-sm font-medium text-gray-900 dark:text-white cursor-pointer">
                      <input
                        type="checkbox"
                        checked={settings.instagram_reels?.enabled ?? false}
                  ...
                  <div className="space-y-3">
                    {(settings.instagram_reels?.links || []).length === 0 && (
                      <p className="text-gray-500 dark:text-gray-400 italic text-sm">No Instagram links added.</p>
                    )}
                    {(settings.instagram_reels?.links || []).map((link, index) => (
                      <div key={index} className="flex gap-2">
                        <input
                          type="url"
                          value={link}
                          onChange={(e) => updateReelLink(index, e.target.value)}
                          placeholder="https://www.instagram.com/reel/..."
                          className="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 text-sm"
                        />
                        <button
                          onClick={() => removeReelLink(index)}
                          className="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
                          title="Remove reel link"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                ...
                <section className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mt-8">
                  ...
                  <div className="space-y-6">
                    {(!settings.showcase || settings.showcase.length === 0) && (
                      <p className="text-gray-500 dark:text-gray-400 italic text-sm">No showcase sections defined. The homepage will skip the showcase section until you add one.</p>
                    )}
                          ...
                          <div className="flex-1 space-y-4">
                            <div>
                              ...
                              <select
                                ...
                                onChange={(e) => {
                                  const newShowcase = [...settings.showcase!];
                                  const oldId = newShowcase[idx].category_id;
                                  const newId = parseInt(e.target.value);
                                  newShowcase[idx].category_id = newId;
                                  if (oldId !== newId) {
                                    delete newShowcase[idx].product_ids_by_category?.[String(oldId)];
                                    delete newShowcase[idx].product_data_by_category?.[String(oldId)];
                                  }
                                  setSettings({ ...settings, showcase: newShowcase });
                                }}
                            ...
                            <div className="bg-white dark:bg-gray-800 p-3 rounded border border-gray-200 dark:border-gray-700 space-y-4">
                              <div>
                                <label className="block text-xs font-medium text-gray-500">Selected Products per Category/Subcategory</label>
                                <p className="text-[11px] text-gray-400">Optional. Select up to 8 products for each tab. If empty, the storefront loads products from that category automatically.</p>
                              </div>

                              {getShowcaseTargets(showcaseItem).map((targetId) => {
                                const key = `${idx}:${targetId}`;
                                const selectedIds = (showcaseItem.product_ids_by_category?.[String(targetId)] || []).map((id: any) => Number(id));
                                const selectedProducts = showcaseItem.product_data_by_category?.[String(targetId)] || [];
                                const results = showcaseSearchResults[key] || [];

                                return (
                                  <div key={targetId} className="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/60 p-3 space-y-3">
                                    <div className="flex items-center justify-between gap-3">
                                      <div>
                                        <p className="text-sm font-semibold text-gray-900 dark:text-white">{findCategoryTitle(targetId)}</p>
                                        <p className="text-[11px] text-gray-500 dark:text-gray-400">{selectedIds.length}/8 products selected</p>
                                      </div>
                                    </div>
                                    ...
                                    <div className="relative">
                                      ...
                                      <input
                                        type="text"
                                        value={showcaseProductSearch[key] || ''}
                                        onChange={(e) => searchShowcaseProducts(key, e.target.value)}
                                        placeholder="Search products by name, SKU or ID..."
                                        className="block w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                                      />
                                      {showcaseSearching[key] && (
                                        <div className="absolute right-3 top-2.5 w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" />
                                      )}
                                    </div>

                                    {results.length > 0 && (
                                      <div className="max-h-52 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-900 divide-y divide-gray-100 dark:divide-gray-800">
                                        {results.map((product: any) => {
                                          const isSelected = selectedIds.includes(Number(product.id));
                                          return (
                                            <button
                                              key={product.id}
                                              type="button"
                                              onClick={() => toggleShowcaseProductSelection(idx, targetId, product)}
                                              className={`w-full flex items-center justify-between gap-3 p-2 text-left hover:bg-blue-50 dark:hover:bg-blue-900/20 ${isSelected ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}
                                            >
                                              <div>
                                                <p className="text-xs font-medium text-gray-900 dark:text-white">{product.name}</p>
                                                <p className="text-[11px] text-gray-500 dark:text-gray-400">SKU: {product.sku || 'N/A'} · ৳{product.sell_price || product.selling_price || product.price || 0}</p>
                                              </div>
                                              <span className={`text-[11px] font-semibold ${isSelected ? 'text-blue-600' : 'text-gray-400'}`}>
                                    ...
                                    {selectedIds.length > 0 && (
                                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                        {selectedIds.map((productId: number) => {
                                          const product = selectedProducts.find((p: any) => Number(p.id) === Number(productId)) || results.find((p: any) => Number(p.id) === Number(productId));
                                          return (
                                            <div key={productId} className="flex items-center justify-between gap-2 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-2 py-1.5">
                                              <div className="min-w-0">
                                                <p className="text-xs font-medium text-gray-900 dark:text-white truncate">{product?.name || `Product #${productId}`}</p>
                                                <p className="text-[10px] text-gray-500 dark:text-gray-400">ID: {productId}</p>
                                              </div>
                                              <button
                                                type="button"
                                                onClick={() => removeShowcaseProduct(idx, targetId, productId)}
